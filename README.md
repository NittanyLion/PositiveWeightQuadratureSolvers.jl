...
| `polish_mp.jl <bank file> [--type x4\|big] [--target …]` | extended-precision polish: minimum-norm Newton on the orthonormal moment system with the residual in `Float64x4` (~64 digits) or 384-bit `BigFloat`, the step from a truncated SVD. Writes the extended twin and, if the Float64 gate improves at least 2×, rewrites the bank file. Needs threads: `julia -t 16` (§29.5–29.7) |
